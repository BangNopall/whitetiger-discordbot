...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const ketuaList = ketuaArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const wakilKetuaList = wakilKetuaArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const bisnisList = bisnisArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const brankasList = brankasArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const jalananList = jalananArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const relasiList = relasiArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
    const anggotaList = anggotaArray
      .map((anggota, index) => `> ${index + 1}. ${anggota.nama}`)
      .join("\n");
