...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("dp-brankas")
    .setDescription("Deposit barang brankas White Tiger Pandawa")
    // buatkan 2 option
    .addStringOption((option) =>
      option
        .setName("barang")
        .setDescription(
          "Pilih barang yang akan di deposit (terdapat 47+ barang)"
        )
        .setRequired(true)
        .setAutocomplete(true)
    )
    .addStringOption((option) =>
      option.setName("jumlah").setDescription("Jumlah deposit barang")
    ),
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      // Tambahkan jumlah deposit barang dan simpan ke database
      brankasData[barang] = (
      ...
      const embed = new EmbedBuilder()
        .setTitle("DP Brankas - White Tiger Pandawa")
        .setDescription(
          `> **Data Deposit Brankas**\n> - User: <@${interaction.user.id}> \n> - Barang : ${barang}\n> - Jumlah : ${jumlah}`
        )
        .setColor("ffffff")
        .setThumbnail(
      ...
      if (interaction.member.roles.cache.has("1155443652911452252")) {
        await interaction.reply({
          content: `Berhasil melakukan deposit ${jumlah} ${barang}`,
          ephemeral: true,
        });
